...
/* -------- Funciones P10 -------- */
function p10_matriz_vacia(): array {
  $ventas = [];
  for ($p=1; $p<=5; $p++) {
    $ventas[$p] = [];
    for ($v=1; $v<=4; $v++) $ventas[$p][$v] = 0.0;
  }
  return $ventas;
}
function p10_acumular(array &$ventas, int $producto, int $vendedor, float $monto): void {
  $ventas[$producto][$vendedor] += $monto;
}
function p10_totales_producto(array $ventas): array {
  $out = [];
  for ($p=1; $p<=5; $p++) { $s = 0.0; foreach ($ventas[$p] as $m) $s += $m; $out[$p] = $s; }
  return $out;
}
function p10_totales_vendedor(array $ventas): array {
  $out = [];
  for ($v=1; $v<=4; $v++) { $s = 0.0; for ($p=1; $p<=5; $p++) $s += $ventas[$p][$v]; $out[$v] = $s; }
  return $out;
}
function p10_gran_total(array $ventas): float {
  $t = 0.0; foreach ($ventas as $fila) foreach ($fila as $m) $t += $m; return $t;
}
...
if ($_SERVER['REQUEST_METHOD']==='POST') {
  ...
  foreach ($v as $i=>$par){
    [$nom,$ape]=$par;
    if (($nom!=='' && !preg_match('/^[\p{L} ]*$/u', $nom)) || ($ape!=='' && !preg_match('/^[\p{L} ]*$/u', $ape)))
      $errores[] = "Nombre/apellido vendedor $i inválido.";
  }

  if (empty($errores)) {
    $ventas = p10_matriz_vacia();
    $lineas = preg_split('/\r\n|\r|\n/', trim($notas));
    foreach ($lineas as $idx=>$linea) {
      if (trim($linea)==='') continue;
      $parts = preg_split('/[;,]/', $linea);
      if (count($parts)!==3){ $errores[]="Línea ".($idx+1).": use vendedor,producto,monto"; continue; }
      [$vendStr,$prodStr,$montoStr] = array_map('trim',$parts);
      $montoStr = str_replace(',', '.', $montoStr);

      if (Utils::esPositivo($vendStr) === false || Utils::esPositivo($prodStr) === false){ $errores[]='Vendedor/producto deben ser enteros positivos.'; continue; }
      $vend=(int)$vendStr; $prod=(int)$prodStr;

      if (!Utils::rangoInt($vend,1,4)){ $errores[]='Vendedor fuera de rango (1..4).'; continue; }
      if (!Utils::rangoInt($prod,1,5)){ $errores[]='Producto fuera de rango (1..5).'; continue; }
      if (!Utils::esFloat($montoStr)){ $errores[]='Monto inválido.'; continue; }

      $monto=(float)$montoStr; if ($monto<0){ $errores[]='Monto no puede ser negativo.'; continue; }
      p10_acumular($ventas,$prod,$vend,$monto);
    }
    ...
    if (empty($errores)) {
      ...
      $rep = [
        'matriz' => $ventas,
        'fila'   => p10_totales_producto($ventas),
        'col'    => p10_totales_vendedor($ventas),
        'total'  => p10_gran_total($ventas),
        'nombres'=> $nombres
      ];
    }
  }
